...
class Database:
    ...
    def find_title_in_tables_goal_with_user_id(user_id, title):
        ...
        cursor = connection.cursor()
        cursor.execute(
            "SELECT money_count FROM goals WHERE user_id = ? AND title = ?",
            [user_id, title],
        )
    ...
    def set_goal(user_id, title, money_count):
        Database.execute(
            "INSERT INTO goals (user_id, title, money_count) VALUES (?, ?, ?)",
    ...
    def delete_completed_goal(user_id, title):
        Database.execute(
            "DELETE FROM goals WHERE user_id = ? AND title = ?", [user_id, title]
        )
        return True
    ...
    def add_income(user_id, title, income):
        Database.execute(
            "INSERT INTO incomes (user_id, title, income) VALUES (?, ?, ?)",
        ...
        cursor = connection.cursor()
        cursor.execute(
            "SELECT savings FROM goals WHERE title = ? AND user_id = ?",
            [title, user_id],
        )
        price_before = int(cursor.fetchall()[0][0])
        cursor.execute(
            "SELECT money_count FROM goals WHERE title = ? AND user_id = ?",
            [title, user_id],
        )
        ...
        Database.execute(
            "UPDATE goals SET savings = ? WHERE title = ? AND user_id = ?",
            [str(price_after), title, user_id],
        )
    ...
    def add_expense(user_id, title, expense):
        Database.execute(
            "INSERT INTO expenses (user_id, title, expense) VALUES (?, ?, ?)",
    ...
    def view_transactions_incomes(user_id, title):
        if title is None:
            ...
            cursor = connection.cursor()
            cursor.execute(
                "SELECT * FROM incomes WHERE user_id = ? AND title = ?",
                [user_id, title],
            )
            incomes = list(cursor.fetchall())
    ...
    def view_transactions_expenses(user_id, title):
        if title is None:
            ...
            cursor = connection.cursor()
            cursor.execute(
                "SELECT * FROM expenses WHERE user_id = ? AND title = ?",
                [user_id, title],
            )
            expenses = list(cursor.fetchall())
    ...
    def statistics(user_id, title):
        if title is None:
            ...
            cursor = connection.cursor()
            cursor.execute(
                "SELECT income FROM incomes WHERE user_id = ? AND title = ?",
                [user_id, title],
            )
            incomes = list(cursor.fetchall())
            incomes_all = 0
            cursor.execute(
                "SELECT expense FROM expenses WHERE user_id = ? AND title = ?",
                [user_id, title],
            )
            expenses = list(cursor.fetchall())
            expenses_all = 0
